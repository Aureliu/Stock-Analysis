...
# Offset is from 0
cout = open('2016-7-29-double-signal-precision.txt', 'w')
...
for i in range(3,2492):
	...
	if ((flag_temp is not flag) or ((i-start_day)>=10 and ((flag_temp and decay_price>start_price) or (not flag_temp and decay_price < start_price)))):
		...
		if(flag_temp==1 and state<4):
			state+=1
			if(table.cell(i+10,2).value>=table.cell(i,2).value):
				num_correct+=1
			else:
				num_wrong+=1
			start_day=i
			...
			money,volumn,num = adjustPosition(money,volumn,table.cell(i,2).value,state,num)
