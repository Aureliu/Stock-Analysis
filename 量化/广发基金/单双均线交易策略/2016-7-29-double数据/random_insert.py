...
# Offset is from 0
cout = open('2016-7-29-double-Dynamic-random-insert-II.txt', 'w')
...
for j in range(1,1001):
	...
	start_day=start
	interest=0.03/365
	volumn=float(0.6)/table.cell(start,2).value
	...
	for i in range(start,nrows):
		...
		if ((flag_temp is not flag) or ((i-start_day)>=10 and ((flag_temp and decay_price>start_price) or (not flag_temp and decay_price < start_price)))):
			...
			if(flag_temp==0 and state>1):
				...
				start_price=table.cell(i,4).value
				money,volumn = adjustPosition(money,volumn,table.cell(i,2).value,state)
			value_list.append(money+volumn*table.cell(i,2).value)
	drawdown= maxDrawdown(value_list)
	start_date=table.cell_value(start,3)
	end_date=table.cell_value(nrows-1,3)
	money=money+volumn*table.cell(nrows-1,2).value
	year=float(end_date-start_date)/365
	gain_model=money**(1/year)
	gain_wind=(table.cell(nrows-1,2).value/table.cell(start,2).value)**(1/year)
	print repr(j)+'\t'+repr(year) + '\t' + repr(table.cell(nrows-1,2)) + '\t' + repr(1/year) + '\t' + repr(gain_model)
